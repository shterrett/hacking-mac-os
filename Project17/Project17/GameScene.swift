...
class GameScene: SKScene {
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        let remaining = 100 - (currentTime - gameStartTime)
        timer.xScale = max(CGFloat(remaining) / 100, 0)
    }
    ...
    override func mouseDown(with event: NSEvent) {
        ...
        if newScore == 1 {
            ...
            score += Int(pow(2, Double(min(16, newScore))))
        }
    }

    func position(for ball: Ball) -> CGPoint {
        return CGPoint(
            x: 72 + ballSize * CGFloat(ball.col),
            y: 50 + ballSize * CGFloat(ball.row)
        )
    }

    func createBall(row: Int, col: Int, startOffscreen: Bool = false) -> Ball {
        let ballImages = ["ballBlue", "ballGreen", "ballRed", "ballGrey"]
    ...
    func ball(at point: CGPoint) -> Ball? {
        return nodes(at: point).flatMap() { $0 as? Ball }.first
    }
